...
func (this *Log) SetLogAdapter(cfg *Config, adapter string) error {
	if adapter == "" {
		return nil
	}

	if a, ok := cfg.Log[adapter]; ok && a.Level > 0 {
		if new_cfg, err := cfg.GetLogAdapterJSON(adapter); err != nil {
			return err
		} else {
			this.DelLogger(adapter)
			this.SetLogger(adapter, new_cfg)
		}
	} else {
		this.DelLogger(adapter)
	}
